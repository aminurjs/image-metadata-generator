  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex gap-6 flex-1 overflow-hidden">
        <div className="w-1/3 bg-card rounded-lg shadow-md overflow-hidden flex flex-col">
          <h2 className="text-sm font-medium p-2 border-b">Explorer</h2>
          <ScrollArea className="flex-1">{displayFolders(fileMap)}</ScrollArea>
        </div>
        <div className="w-2/3 bg-card rounded-lg shadow-md overflow-hidden flex flex-col">
          <h2 className="text-sm font-medium p-2 border-b">
            {selectedPath || "Select a folder"}
          </h2>
          <ScrollArea className="flex-1 p-4">{getSelectedContent()}</ScrollArea>
        </div>
      </div>
...
function FolderIcon({
  ...
  const getIconColor = () => {
    switch (fileType?.toLowerCase()) {
      ...
      default:
        return "currentColor"; // Use the current text color
    }
  };
